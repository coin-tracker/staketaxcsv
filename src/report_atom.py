...
import staketaxcsv.atom.cosmoshub3.api_cosmostation
...
def txone(wallet_address, txid):
    if localconfig.legacy:
        elem = staketaxcsv.atom.cosmoshub3.api_cosmostation.get_tx(txid)
    else:
...
def txhistory(wallet_address, options):
    ...
    # Fetch transactions
    elems.extend(staketaxcsv.common.ibc.api_lcd.get_txs_all(ATOM_NODE, wallet_address, progress, max_txs, debug=localconfig.debug))
...
def _fetch_txs_legacy(wallet_address, progress):
    ...
    for _ in range(0, _max_pages()):
        ...
        elems, next_id = staketaxcsv.atom.cosmoshub3.api_cosmostation.get_txs_legacy(wallet_address, next_id)
        out.extend(elems)
        if next_id is None:
            break
